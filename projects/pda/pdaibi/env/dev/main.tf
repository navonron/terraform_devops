...
module "dev" {
  ...
  sa = [
    {
      subresources = ["queue", "file", "blob", "table", "dfs"]
      is_hns       = true
      ...
      lifecycle = [{
        name                                          = "ibi_daas_query_cycle24h"
        blob_types                                    = "blockBlob"
        prefix_match                                  = ["ibi-daas-query/"]
        delete_after_days_since_creation_greater_than = 1
      }]
    },
    {
      subresources = ["blob", "queue", "file", "table"]
      workload     = "func"

      tags = {
        "target-resource-type" = "windows_function_app"
        "target-resource-name" = "arm, collectionflow, policyorch, persistence"
      }
      ...
      rbac = [
        {
          ...
          members = concat([
            data.azuread_groups.ad_groups.object_ids[0]],
            data.azuread_service_principals.service_principals.object_ids[*]
          )
        },
        {
          role    = "Storage Blob Delegator"
        ...
        },
        {
          ...
          members = [data.azuread_service_principals.service_principals.object_ids[0]]
        }
      ]
    }
  ]
  ...
  synapse = [{
    ...
    sa = {
      workload     = "synw"
      subresources = ["file", "table", "blob", "queue", "dfs"]
    }
